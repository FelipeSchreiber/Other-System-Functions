...
#include <unistd.h>
#include <sys/types.h>
#include "mldlUmlApproveRegistrationRequest.h"
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
mldlUmlErrorType
MldlUmlApproveRegistrationRequest (mldlUmlConfigurationOptionsType *mldlUmlConfigurationOptions, char *mldlUmlUsername, 
											char *mldlUmlRequesterUsername)
{
	...
	if (mldlUmlReadingAuxiliary.mldlUmlExpirationDate < time(NULL))	/*confusion*/
	...
	while (fgets(mldlUmlBuffer, sizeof(mldlUmlBuffer), mldlUmlRead))
	{
		...
		if (!(strcmp(mldlUmlStringCompare, mldlUmlRequesterUsername)))
		{
			...
			mldlUmlSecondWritingAuxiliary = (char *) malloc(strlen(mldlUmlWritingAuxiliary) + 
														strlen(mldlUmlReadingAuxiliary.mldlUmlRequestingUserPassword) + 
														strlen(mldlUmlDoubleDotPosition) + 2);

			snprintf(mldlUmlSecondWritingAuxiliary, (strlen(mldlUmlWritingAuxiliary) + 
																strlen(mldlUmlReadingAuxiliary.mldlUmlRequestingUserPassword) 
																+ strlen(mldlUmlDoubleDotPosition) + 2), "%s%s%s", mldlUmlWritingAuxiliary, 
																mldlUmlReadingAuxiliary.mldlUmlRequestingUserPassword, (mldlUmlDoubleDotPosition + 1));
